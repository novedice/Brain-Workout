...
export const createResult = async (resData: IResData) => {
  try {
    const response = await $host.post<IResultResponse>(`${BaseUrl}/${users}/${results}`, resData, {
      withCredentials: true,
    });
...
export const getBestResult = async (gameId: number, sort: 'ASC' | 'DESC') => {
  try {
    const response = await $host.get<{value: number}>(`${BaseUrl}/${users}/${results}/best?gameId=${gameId}&sort=${sort}`, {
      withCredentials: true,
    });
    return response.data;
  } catch (e) {
